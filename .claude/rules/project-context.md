...
A repository of design tokens stored as JSON. `dist/tokens.json` is the single source of truth for the entire **Lenta Omni Support Admin** design system, covering both Figma and the frontend.
...
```
src/<N>-<name>.json              (hand-edited sources, merged in numeric-prefix order)
        │
        ▼  scripts/merge-tokens.js (deep merge, plain Node, no deps)
        ▼
dist/tokens.json                  (generated — the single source of truth)
        │
        ▼  Computed Variables Figma plugin (external project, see below)
        ▼
Figma Variables  ──────────────────────────────►  Frontend (Vuetify)
        │
        ▼  scripts/vuetify-theme/
        ▼
dist/vuetify-theme.json (alternate path: resolves dist/tokens.json directly, without Figma)
```
